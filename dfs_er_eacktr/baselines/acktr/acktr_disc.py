...
class Runner(object):
    def __init__(self, env, model, config, nsteps=5, gamma=0.99):
        self.env = env
    ...
    def run(self):
        ...
        for n, (rewards, dones, actions, value) in enumerate(zip(mb_rewards, mb_dones, mb_actions, last_values)):
            rewards = rewards.tolist()
            dones = dones.tolist()
            actions = actions.tolist()
            #print(dones)
            if dones[-1] == 0:
                rewards = discount_with_dones(rewards+[value], dones+[0], actions+[0], self.gamma, self.nact, self.config)[:-1]
            else:
                rewards = discount_with_dones(rewards, dones, actions, self.gamma, self.nact, self.config)
...
class Model(object):
    def __init__(self, policy, ob_space, ac_space, nenvs,
        ...
        step_model = policy(dataflow_config, sess, ob_space, ac_space, nenvs, 1, reuse=False)
        eval_step_model = policy(dataflow_config, sess, ob_space, ac_space, 1, 1, reuse=True)
        train_model = policy(dataflow_config, sess, ob_space, ac_space, nenvs*nsteps, nsteps, reuse=True)
        expert_train_model = policy(dataflow_config, sess, ob_space, ac_space, expert_nbatch, 1, reuse=True)
...
def learn(policy, env, seed, params,
    ...
    runner = Runner(env,  model, dataflow_config, nsteps=nsteps, gamma=gamma)
    ...
    while True:
        ...
        if now - t_last_vid > video_interval:
            easy_video(model, params, 'prob', dataflow_config)
