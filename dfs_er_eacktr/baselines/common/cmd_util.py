    ...
    def make_env(rank): # pylint: disable=C0111
        def _thunk():
            if random_state_reset:
                ...
                env = make_atari(env_id, dataflow_config)
            env.seed(seed + rank)
...
def make_eval_env(config, env_id, dumpdir=None, wrapper_kwargs=None, seed=None):
    if wrapper_kwargs is None: wrapper_kwargs = {}
    wrapper_kwargs['is_monte'] = 'MontezumaRevenge' in env_id
    wrapper_kwargs['is_pong'] = 'Pong' in env_id
    env = make_atari(env_id, config)
    if seed is not None:
        env.seed(seed)
...
def atari_arg_parser():
    ...
    parser.add_argument('--num_env', help="how many parallel envs to do", type=int, default=int(32))
    parser.add_argument('--nsteps', help="number of steps after which we boostrap by value function", type=int, default=20)
    parser.add_argument('--expert_nbatch', help="batch size of expert data", type=int, default=int(256) )
