...
class NoopResetEnv(gym.Wrapper):
    ...
    def step(self, ac):
        #print("NoopResetEnv : action :" + str(ac))
...
class FireResetEnv(gym.Wrapper):
    ...
    def step(self, ac):
        #print("FireResetEnv : action :" + str(ac))
        return self.env.step(ac)
...
class MaxAndSkipEnv(gym.Wrapper):
    def __init__(self, env, config=None, skip=4):
    ...
    def step(self, action):
        ...
        #print("max and skip env")
        total_reward = 0.0
        done = None
        #print(action)
        #print("nact :" + str(self.nact))
        #print(self.DFS_ER)
        #print(self._r2skip)
        if(self.DFS_ER & (action >= self.nact)):
            for i in range(self._r2skip):
                #print("skip env : action" + str(action))
                obs, reward, done, info = self.env.step(action - self.nact)
                if i == self._r2skip - 2: self._obs_buffer[0] = obs
                if i == self._r2skip - 1: self._obs_buffer[1] = obs
                total_reward += reward
                if done:
                    break            
        else:    
            for i in range(self._skip):
                #print("skip env : action else" + str(action))
                obs, reward, done, info = self.env.step(action)
...
def make_atari(env_id, config=None):
    env = gym.make(env_id)
    assert 'NoFrameskip' in env.spec.id
    env = NoopResetEnv(env, noop_max=30)
    env = MaxAndSkipEnv(env, config, skip=4)
    return env
