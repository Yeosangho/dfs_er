...
class ExpertDataflow(RNGDataFlow):
    ...
    """
    def __init__(self, config, n_act):
        self.n_act = n_act
        self.img_dir = config['img_dir']
        self.config = config
        self.DFS_ER = config['DFS-ER']
        self.r2_cond = config['r2_cond']
        self.r2_skip = config['r2_skip']
        ...
        self.rewards = []
        print(len(self.img_df))

        #for i in range(len(self.img_df)):
        #        self.rewards.append(self.get_nstep_reward(i, self.img_df))
        #self.img_df.loc[:, 'testrewards'] = self.rewards 
    ...
    def load_stacked_imgs(self, current):
        ...
        frames = [] * self.config['frame_history']
        target = current
        data_row = self.img_df.iloc[target]
        pre_action_repeat = data_row.action_repetitions
        # make processed image stack
        for i in range(self.config['frame_history']):
            # -1, because we take _previous_ observation to predict action

            img_one = self._load_image(target - 1)
            img_two = self._load_image(target - 2)
            #print("i :" + str(i))
            if(self.DFS_ER) :
                target = target - self.config['r2_skip']
                data_row = self.img_df.iloc[target]
                action_repeat = data_row.action_repetitions
                if((action_repeat ==  pre_action_repeat + self.config['r2_skip']) & (action_repeat >= self.config['r2_cond'])):
                    None
                    #print("target skip r2!!")
                else:
                    target = target + self.config['r2_skip'] - self.config['frameskip']
                    data_row = self.img_df.iloc[target]
                    action_repeat = data_row.action_repetitions
                    #print("target skip r1!!")
                pre_action_repeat = action_repeat
                #print("action repeat : " + str(action_repeat))
                #print("pre_action_repeat : " + str(pre_action_repeat))
            else:
                target = target - self.config['frameskip']
        ...
        imgs = np.concatenate(states, axis=2)

        #print(imgs.shape)
        return imgs
    def get_nstep_reward(self, current, idxs):
        ...
        for i in range(self.config['n_step']):
            ...
            data_row = self.img_df.iloc[current]
            action_repeat = data_row.action_repetitions
            if(action_repeat >= self.config['r2_cond']):
                    current = current + self.config['r2_skip']
                    reward = reward + (self.gamma**real_steps)*(data_row.r2_rewards.clip(-1,1))
                    real_steps = real_steps + (self.config['r2_skip']/self.config['frameskip'])
            else:
                    current = current + self.config['frameskip']
                    reward = reward + (self.gamma**real_steps)*(data_row.r1_rewards.clip(-1,1))
                    real_steps = real_steps + (self.config['frameskip']/self.config['frameskip'])  
        return reward            
    ...
    def get_data(self):
        ...
        while True:
            ...
            #future_rewards = self.get_nstep_reward(current, idxs)
            #print("curent :" + str(current))
            #print("len : " + str(len(self.rewards)))
            #future_rewards = data_row.testrewards
            action_label = data_row.action
            #print(future_rewards)
            action_repeat = data_row.action_repetitions
            if(self.DFS_ER):
                if(action_repeat >= self.r2_cond):
                    action_label = action_label + self.n_act
            #print("action_label :" + str(action_label) + "n_act : " + str(self.n_act))        
           # print(data_row.action_repetitions)
...
def get_dataflows(config, n_act):
    """
    construct and initialize dataflows based on config.
    """

    df = ExpertDataflow(config, n_act)
